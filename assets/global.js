...
/* Search move */
class SearchMove extends HTMLElement {
  connectedCallback() {
    const listElement = this.querySelector("ul");
    if (!listElement) return;

    const listItems = listElement.querySelectorAll("li");
    const itemHeight = 27; // must match CSS height
    ...
    const scrollDistance = itemHeight * numberOfItems;

    const scrollSpeedPixelsPerSecond = 60;
    const minimumDurationSeconds = 5;
    const maximumDurationSeconds = 20;

    let calculatedDuration = scrollDistance / scrollSpeedPixelsPerSecond;

    const animationDurationSeconds = Math.min(
      Math.max(calculatedDuration, minimumDurationSeconds),
...
customElements.define("search-move", SearchMove);


// color swatches
// if (!window.cardProductSwatchesInit) {
//   window.cardProductSwatchesInit = true;

//   document.addEventListener('pointerover', (event) => {
//     const more = event.target.closest('.card-product-swatches-more');

//     if (more) {
//       more.closest('[data-card-swatches]')?.classList.add('is-expanded');
//       return;
//     }
...
//       card.classList.add('is-swatch-hover');
...
//     if (swatches && !swatches.contains(event.relatedTarget)) {
...
//       card.classList.remove('is-swatch-hover');
...
if (!window.cardProductSwatchesInit) {
  ...
  const isMobileSwatchView = () => window.matchMedia('(max-width: 992px)').matches;
  ...
  document.addEventListener('click', (event) => {
    ...
    if (isMobileSwatchView()) {
      more.closest('[data-card-swatches]')?.classList.toggle('is-expanded');
    }
  }, true);

  document.addEventListener('pointerover', (event) => {
    if (isMobileSwatchView()) return;
  ...
  document.addEventListener('click', (event) => {
    ...
    if (!event.target.closest('[data-card-swatches]')) {
      document.querySelectorAll('[data-card-swatches].is-expanded').forEach((swatches) => {
        swatches.classList.remove('is-expanded');
      });
    }
  });
